		...
		<main id="main" class="<?php echo esc_attr( barcelona_main_class() ); ?>">
			...
			<?php if ( have_posts() ): while( have_posts() ): the_post(); ?>
				...
				<article id="post-<?php echo intval( get_the_ID() ); ?>" <?php post_class(); ?>>
                                    ...
                                    <?php endif; ?>
					<?php		// Gaëlle
					echo get_the_tag_list('<div><span class="libelles-blocs">Destinations </span><span class="lien-categorie">','</span>, <span class="lien-categorie">','</span></div>');
					...
					if ( ! empty( $categories ) ) {
					    foreach( $categories as $category ) {
					        $output .= ' <a class="lien-categorie" href="' . esc_url( get_category_link( $category->term_id ) ) . '" alt="' . esc_attr( sprintf( __( 'View all posts in %s', 'textdomain' ), $category->name ) ) . '">' . esc_html( $category->name ) . '</a>' . $separator;
					    }
							echo '<div><span class="libelles-blocs">Sujets</span>';
