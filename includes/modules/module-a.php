...
while ( $barcelona_q->have_posts() ): $barcelona_q->the_post();
	...
	$barcelona_col_classes = ( $barcelona_counter == 0 ) ? array( 'col-sm-8 col-left mas-item', 'psum-featured' ) : array( 'col col-sm-12 col-xs-6', 'psum-small' );
	$barcelona_col_classes[1] .= ' post-format-'. sanitize_html_class( barcelona_get_post_format() );

	if ( $barcelona_counter == 1 ) {
		echo '<div class="col-sm-4 col-right mas-item"><div class="row">';
	}
		...
		<article class="post-summary <?php echo esc_attr( $barcelona_col_classes[1] ); ?>">
			...
			<div class="post-image">
				...
				<a href="<?php echo esc_url( get_the_permalink() ); ?>" title="<?php echo esc_attr( get_the_title() ); ?>">
					<?php barcelona_psum_overlay(); barcelona_thumbnail( $barcelona_counter == 0 ? 'barcelona-md' : 'barcelona-sm' ); ?>
				</a>
			...
			<div class="post-details">
				...
				<?php
				if ($barcelona_counter == 0 && is_array( $barcelona_mod_post_meta ) && ! empty( $barcelona_mod_post_meta ) && in_array( 'excerpt', $barcelona_mod_post_meta ) ) : ?>
